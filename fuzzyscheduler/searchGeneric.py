...
class BFS_searcher(Searcher):
    ...
    def add_to_expended_state(self,state):
        self.expended_state.append(state)
    ...
    def search(self):
        ...
        while not self.empty_frontier():
            ...
            self.display(3, "Neighbors are", neighs)
            for arc in neighs:
                if(arc.to_node not in self.expended_state) or (arc.to_node in self.frontier_state):
                    self.add_to_frontier(Path(path, arc))
                    self.frontier_state.remove(path.end())
                    self.display(3, "Frontier:", self.frontier)
